...
function f_createBooking(matchId, visitorId, isRedeemed, verificationCode) { //Andere Parameter noch einfügen?
    n_id = bookings.length;
   // In Zeile 60 übergebene Parameter werden in DB eingefügt 
    db.run('Insert into booking (ID,MatchDay,VerificationCode,Email,Telephone,IsRedeemed) values (?,?,?,?,?;?) ', matchId,matchDay,verificationCode,email,telephone,isRedeemed);
        if (err) {
...
function f_getBooking(id) {
    db.run('Select * from Booking where id ='+ id +'');{
        if (err) {
            console.error(err.message);
          }
...
function f_deleteBooking(id) {
    db.run('Delete * from Booking where id ='+ id +'');{
        if (err) {
            console.error(err.message);
          }
...
const sqlite3 = require('sqlite3').verbose();
let db = new sqlite3.Database('C:/Users/Marcel/Downloads/sqlite-tools-win32-x86-3340000/sqlite-tools-win32-x86-3340000/SE.db', (err) => {
    if (err) {
      console.error(err.message);
    }
